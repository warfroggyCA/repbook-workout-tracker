...
export function readLines(input: string): Line[] {
  const result: Line[] = [];
  let separated = true;
  for (const source of input.split(/\r?\n/)) {
    ...
    if (
      ...
      !command &&
      result.length &&
      !result[result.length - 1].text.endsWith(":") &&
...
function nameTokens(value: string) {
  const normalized = normalizeExerciseText(
    value.replace(/^(?:the\s+)?loaded\s+/i, ""),
  );
  ...
  return (shorthand[normalized] ?? normalized)
...
export function sameMovement(reference: string, name: string): boolean {
...
function preparationItem(text: string): Warmup | null {
  ...
  if (numeric) {
    if (
      Number(numeric[1]) > MAX_STORED_LOAD ||
      normalizeStoredLoad(Number(numeric[1])) !== Number(numeric[1]) ||
...
export function parseProgramTextUpdate(
  ...
  for (const line of lines) {
    ...
    if (section === "rules") {
      if (
        /^(?:change|set|update|add|replace|remove|move|reorder|group|ungroup)\b/i.test(
          text,
        )
      ) {
        question(
          `This is a Program command inside preparation guidance: “${text}”. Separate it from the warm-up request before applying changes.`,
        );
        continue;
      }
    ...
    if (
      /^(?:warm[ -]?up update|update warm[ -]?up only|do not duplicate)/i.test(
        text,
      )
    ) {
    ...
    if (/^(?:keep|leave)\b/i.test(text)) {
      if (
        /^(?:keep|leave)\s+(?:working exercises, working sets, progression rules and working-set supersets|all working prescriptions|everything else|every unmentioned day and exercise)\s+unchanged[.]?$/i.test(
          text,
        )
      )
        continue;
      if (/\b(?:warm[ -]?up|preparation)\b/i.test(text)) {
        addNote(globalNotes, text);
        continue;
      }
      question(
        `The preservation instruction needs clarification: “${text}”. It cannot be verified from this preparation block.`,
      );
      continue;
    }
    const editCommand =
