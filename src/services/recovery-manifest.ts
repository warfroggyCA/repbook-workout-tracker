...
export const RECOVERY_TABLE_MANIFEST = [
  ...
  entry("session_exercises", "Workout exercises", "parent_chain", "session_exercises.session_id -> workout_sessions.user_id", ["workout_sessions", "exercises", "session_exercise_groups"], "canonical", "replace", "replace", linked, cascade, durable, ["workout reference", "exercise reference", "load unit", "same-workout group membership", "stable source slot lineage", "immutable nullable prescribed name, metric, load type, and load semantics snapshot", "immutable nullable retained equipment-requirements snapshot with source identities and labels"]),
  ...
  entry("audit_logs", "Audit log", "operational_user", "audit_logs.user_id", ["users", "record_versions", "workout_sessions"], "canonical", "merge", "merge", preserve, "snapshot restore merges only immutable workout-timing audits whose version evidence is also merged and appends an owner-confirmed restore transition when an earlier completed-workout timing fact becomes effective; other audit rows remain preserved at the destination; deletion otherwise follows the account", "durable_audit", ["owner", "immutable identity", "workout-timing version link", "snapshot timing-lineage merge", "monotonic workout-timing restore transition"]),
