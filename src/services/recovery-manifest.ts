export const RECOVERY_MANIFEST_VERSION = 16 as const;
...
export const RECOVERY_TABLE_MANIFEST = [
  ...
  entry("equipment_items", "Equipment", "direct_user", "equipment_items.user_id", ["users", "equipment_definitions"], "canonical", "replace", "preserve", preserve, cascade, durable, ["owner", "definition reference", "explicit cable pulley capability"]),
  ...
  entry("plate_loaded_machine_profiles", "Plate-loaded machine profiles", "direct_user", "plate_loaded_machine_profiles.user_id", ["users", "equipment_items"], "canonical", "replace", "preserve", preserve, cascade, durable, ["owner", "exact equipment item", "known geometry completeness", "versioned total added plate meaning without assumed unloaded resistance"]),
  ...
  entry("programs", "Programs", "direct_user", "programs.user_id", ["users"], "canonical", "replace", "dependency", linked, cascade, durable, ["owner", "one active Program plus saved inactive Programs", "usable Programs have a current immutable version"]),
  ...
  entry("program_schedules", "Program schedules", "direct_user", "program_schedules.user_id", ["users", "programs"], "canonical", "replace", "preserve", preserve, cascade, durable, ["owner", "Program reference", "current immutable schedule version reconciled after version insertion"]),
  entry("program_schedule_versions", "Program schedule versions", "direct_user", "program_schedule_versions.user_id", ["users", "program_schedules", "programs", "program_versions"], "canonical", "replace", "preserve", preserve, cascade, durable, ["owner", "schedule root", "immutable source Program version", "strict version lineage", "validated fixed or rolling phase document", "canonical content hash"]),
  entry("scheduled_program_events", "Scheduled Program events", "direct_user", "scheduled_program_events.user_id", ["users", "program_schedules", "program_schedule_versions", "program_versions", "workout_templates"], "canonical", "replace", "preserve", preserve, cascade, durable, ["owner", "exact schedule and Program version", "stable phase and event identity", "resistance routine lineage", "local calendar and timezone", "schedule-decision status and revision", "not performed-work evidence"]),
  ...
  entry("workout_sessions", "Workouts", "direct_user", "workout_sessions.user_id", ["users", "programs", "program_versions", "workout_templates", "history_import_batches", "archive_operations"], "canonical", "replace", "replace", linked, cascade, durable, ["owner", "timezone/local date", "one active workout", "owner-scoped immutable Start request identity and canonical payload hash", "history revision", "performed-time precision", "nullable all-or-none active-duration evidence separate from source timestamps", "nullable versioned planned-duration evidence distinct from the session cap", "nullable versioned terminal completion state and explicit reason", "all-or-none source Program lineage", "self-contained immutable Program schedule snapshot that never depends on current schedule rows", "snapshotted day warm-up guidance", "immutable accepted compiler provenance"]),
  ...
  entry("session_exercises", "Workout exercises", "parent_chain", "session_exercises.session_id -> workout_sessions.user_id", ["workout_sessions", "exercises", "session_exercise_groups"], "canonical", "replace", "replace", linked, cascade, durable, ["workout reference", "exercise reference", "load unit", "same-workout group membership", "stable source slot lineage", "immutable nullable prescribed name, metric, load type, and load semantics snapshot", "immutable nullable prescribed-counting tuple bound to an eligible retained repetition metric", "immutable nullable retained equipment-requirements snapshot with source identities and labels", "coherent skip and substitution decision evidence with busy distinct from unavailable or incompatible"]),
  ...
  entry("recommendations", "Recommendations", "direct_user", "recommendations.user_id", ["users", "exercises", "progression_jobs", "workout_template_exercises", "coaching_insights", "archive_operations"], "canonical", "replace", "merge", linked, cascade, durable, ["owner", "source references", "decision consistency", "history restore never downgrades a terminal recommendation to pending", "versioned evidence and proposed future effect", "review and defer revisions", "coherent durable defer state", "external-AI proposal references a same-owner validated import receipt"]),
  ...
  entry("coaching_insights", "Coaching insights", "direct_user", "coaching_insights.user_id", ["users", "workout_sessions", "session_exercises", "completed_sets", "coaching_insights", "archive_operations"], "canonical", "replace", "replace", linked, cascade, durable, ["owner", "reply graph", "generation state", "minimal retained context", "external-analysis import identity, selected allowlisted observations, source bindings, and no raw response retention"]),
  ...
  entry("audit_logs", "Audit log", "operational_user", "audit_logs.user_id", ["users", "record_versions", "workout_sessions"], "canonical", "merge", "merge", preserve, "snapshot restore merges immutable workout-timing audits whose version evidence is also merged, exact completed-workout Finish command receipts, and an owner-confirmed transition when an earlier completed-workout timing fact becomes effective; other audit rows remain preserved at the destination; deletion otherwise follows the account", "durable_audit", ["owner", "immutable identity", "workout-timing version link", "snapshot timing-lineage merge", "monotonic workout-timing restore transition", "exact Finish command receipt merge"]),
  ...
  entry("record_versions", "Version history", "operational_user", "record_versions.user_id", ["users"], "canonical", "merge", "merge", preserve, "T02 merges performed-set chains and 0079 merges workout-timing chains while appending a monotonic transition for any effective completed-workout timing rollback; other version types remain preserved at the destination until their own bridge contract; deletion otherwise follows the reviewed entity or account", durable, ["owner", "immutable identity", "version chain", "performed-set snapshot lineage merge", "workout-timing snapshot lineage merge", "session-exercise before and after decision evidence", "effective-row agreement after restore"]),
