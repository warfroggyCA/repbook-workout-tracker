...
test("retains a paused nonzero position through mode change and expanded preview", async ({ page }) => {
  await page.goto("/sign-in");
  await page.getByPlaceholder("allowlisted email").fill("owner@example.com");
  const login = page.getByRole("button", { name: "Dev login", exact: true });
  await waitForHydratedServerAction(login);
  ...
  await expect.poll(() => video.evaluate((v: HTMLVideoElement) => v.currentTime)).toBeGreaterThan(.1);
  await player.getByRole("button", { name: "Pause form animation", exact: true }).click();
  const cue = await player.locator("[data-form-banner]").getAttribute("data-form-banner");
  await player.getByText("Playback options", { exact: true }).click();
  const position = player.getByRole("slider", { name: "Demonstration position" });
  await position.focus();
  await position.press("End");
  await position.press("ArrowLeft");
  await expect.poll(() => video.evaluate((v: HTMLVideoElement) => v.currentTime)).toBeCloseTo(5.96, 2);
  await player.getByLabel("Muscle highlights").selectOption("steady");
  await expect(video).toHaveAttribute("src", /steady\.mp4$/);
  await expect.poll(() => video.evaluate((v: HTMLVideoElement) => v.currentTime)).toBeCloseTo(5.96, 2);
  await expect.poll(() => video.evaluate((v: HTMLVideoElement) => v.paused)).toBe(true);
  await expect(player.locator("[data-form-banner]")).toHaveAttribute("data-form-banner", cue!);
  await page.getByRole("button", { name: "Expand form preview", exact: true }).click();
  const expanded = page.getByRole("dialog", { name: "Incline Dumbbell Curl · Form" });
  await expect(expanded).toBeVisible();
  await expect(expanded.locator("video")).toHaveAttribute("src", /steady\.mp4$/);
  await expect.poll(() => expanded.locator("video").evaluate((v: HTMLVideoElement) => v.currentTime)).toBeCloseTo(5.96, 2);
  await expect.poll(() => expanded.locator("video").evaluate((v: HTMLVideoElement) => v.paused)).toBe(true);
  await expect(expanded.locator("[data-form-banner]")).toHaveAttribute("data-form-banner", cue!);
