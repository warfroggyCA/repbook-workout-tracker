...
import type { SessionExerciseData, SessionOccurrenceData } from "@/components/session/types";
...
const occurrence = {
  kind: "working_set",
  kindOrdinal: 1,
} as unknown as SessionOccurrenceData;
...
describe("WorkoutStatusBar", () => {
  ...
  it("keeps an appended performance outside the planned-set denominator", () => {
    const html = renderToStaticMarkup(
      <WorkoutStatusBar
        exercise={exercise}
        occurrence={{ ...occurrence, kindOrdinal: 3 }}
        position={{
          kind: "extra",
          number: 1,
          label: "Extra set 1",
