...
const port = Number.parseInt(process.env.STAGE7_PORT ?? "3117", 10);
const evidenceRoot = resolve("output/playwright");
const outputRoot = resolve(evidenceRoot, runId);

if (!Number.isInteger(port) || port < 1024 || port > 65535) {
  throw new Error("STAGE7_PORT must be an integer from 1024 through 65535.");
}
if (!/^remediation-\d{4}-\d{2}-\d{2}-stage7-r\d+$/.test(runId)) {
  throw new Error("STAGE7_RUN_ID must use remediation-YYYY-MM-DD-stage7-rN.");
}
if (relative(evidenceRoot, outputRoot).startsWith("..")) {
  throw new Error("STAGE7_RUN_ID must stay inside output/playwright.");
}
if (process.env.TEST_WORKER_INDEX === undefined && existsSync(outputRoot)) {
  throw new Error(
    `Stage 7 evidence already exists for ${runId}; choose the next unused revision.`,
  );
}

export default defineConfig({
  testDir: "./tests/e2e",
  testMatch: ["stage7-residual-ux.spec.ts"],
